...
from crawlers.core.abc.plugin import CrawlerPlugin
from crawlers.core.abc.processor import Processor
from crawlers.core.abc.workspace import RunContext, make_run_dir
...
from crawlers.core.processors.parsers import Parser, ParserProcessor
...
class CrawlSpec:
    ...
    Optional fields:
        ...
        max_items: Cap on items to process; enables progress bar if set
...
class DefaultCrawlerPlugin(CrawlerPlugin):
    ...
    def __init_subclass__(cls, **kwargs: Any) -> None:
        ...
        if isinstance(cls.__dict__.get("name"), str) and "crawl" not in cls._commands:
            ...
            from crawlers.core.abc.plugin import CommandDef
    ...
    async def run_crawl(self, config: DefaultCrawlConfig) -> None:
        ...
        try:
            async with spec.client:
                ...
                pipeline = self.build_pipeline(spec, ctx)
    ...
    def _create_run_context(
        self, config: DefaultCrawlConfig, spec: CrawlSpec
    ) -> DefaultRunContext:
