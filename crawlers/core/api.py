...
type ApiFailure = HttpFailure | TimeoutFailure
...
class ApiClient[OptsT, DatasetT](ABC):
    ...
    async def get_json(self, url: str) -> Result[dict, ApiFailure]:
        """
    ...
    async def post_json(self, url: str, body: dict) -> Result[dict, ApiFailure]:
        """
    ...
    async def validate_url(self, url: str) -> Result[bool, ApiFailure]:
        """
        ...
        try:
            async with self.session.head(url, allow_redirects=True) as resp:
                ...
                return Err(HttpFailure(status=resp.status, method="HEAD", url=url))
        except (aiohttp.ClientError, asyncio.TimeoutError) as exc:
            return Err(
                TimeoutFailure(method="HEAD", url=url, attempts=1, last_error=str(exc))
            )
